...
def train(opt, train_loader, G, D, board):
    ...
    for step in range(opt.n_iter):
        ...
        for t in range(opt.Diters):
            ...
            y1 = torch.ones(current_batch_size).cuda()
            y2 = torch.zeros(current_batch_size).cuda()
            # Detach y_pred_fake from the neural network G and put it inside D
            ya_pred_fake = D(pa_fake.detach())

            errD = (BCE_stable(ya_pred - torch.mean(ya_pred_fake), y1) +
                    BCE_stable(ya_pred_fake - torch.mean(ya_pred), y2)) / 2.0
            ...
            x_both = pa.data * u + pa_fake.data * (1. - u).cuda()
            ...
            errD = (BCE_stable(ya_pred - torch.mean(yb_pred_fake), y1) +
                    BCE_stable(yb_pred_fake - torch.mean(ya_pred), y2)) / 2.0
        ...
        for t in range(opt.Giters):
            ...
            l_warp = 15 * criterionWarp(warped_cloth_a, parse_cloth)  # 乘15否则warp cloth效果太差
            l_perceptual = criterionPerceptual(pa_fake, pa)
            l_L1 = criterionL1(pa_fake, pa)
            ya_pred = D(pa)
            ya_pred_fake = D(pa_fake) 
            y1 = torch.ones(current_batch_size).cuda()
            y2 = torch.zeros(current_batch_size).cuda()
            # Non-saturating RaLoss
            l_adv1 = (BCE_stable(ya_pred - torch.mean(ya_pred_fake), y2) +
                      BCE_stable(ya_pred_fake - torch.mean(ya_pred), y1)) / 2
            ...
            # Non-saturating RaLoss            
            l_adv2 = (BCE_stable(ya_pred - torch.mean(yb_pred_fake), y2) +
                      BCE_stable(yb_pred_fake - torch.mean(ya_pred), y1)) / 2
            
            l_adv = 0.1 * (l_adv1 + l_adv2)
            ...
            if (step + 1) % opt.display_count == 0:
                ...
                board.add_scalar('l_D', errD.item() + grad_penalty.item(), step + 1)
                board.add_scalar('loss', loss.item(), step + 1)
                t = time.time() - step_start_time
                print('step: %8d, time: %.3f, loss: %4f, l_warp: %.4f, l_vgg: %.4f, l_L1: %.4f, l_adv: %.4f, l_D: %.4f'
                      % (step + 1, t, loss.item(), l_warp.item(), l_perceptual.item(), l_L1.item(), 
                         l_adv.item(), errD.item() + grad_penalty.item()),
                      flush=True)
                step_start_time = time.time()
